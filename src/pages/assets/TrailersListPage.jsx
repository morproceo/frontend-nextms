/**
 * TrailersListPage — mirrors the Trucks list design exactly:
 *   - KPI strip on top (compact 4-col mobile, 4-col desktop)
 *   - Smart search with clear-X and live count
 *   - Status + Type dropdown pills
 *   - Row: accent avatar + unit + type pill + meta + status pill
 */
...
import {
  Container, Plus, Search, Truck, Snowflake, ChevronRight, ChevronDown,
  AlertTriangle, Wrench, X, Activity, Layers
} from 'lucide-react';
...
export function TrailersListPage() {
  ...
  const handleAddTrailer = () => navigate(orgUrl('/assets/trailers/new'));
  const handleTrailerClick = (id) => navigate(orgUrl(`/assets/trailers/${id}`));
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-title font-semibold text-text-primary">Trailers</h1>
          <p className="text-body-sm text-text-secondary mt-1 hidden sm:block">
            Manage your fleet's trailers and equipment
          </p>
        </div>
        <Button onClick={handleAddTrailer} className="shrink-0">
      ...
      {allTrailers.length > 0 && (
        <div className="grid grid-cols-4 lg:grid-cols-4 gap-2 sm:gap-3">
          <KpiCard icon={Container} label="Total"     value={stats.total} />
          <KpiCard icon={Activity}  label="Active"    value={stats.active}    tone="green" />
          <KpiCard icon={Wrench}    label="Available" value={stats.available} tone="accent" />
          <KpiCard icon={Snowflake} label="Reefers"   value={stats.reefers}   tone="blue" />
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
            ...
            <span className="text-small text-text-tertiary tabular-nums">
              {trailers.length}/{allTrailers.length}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:flex gap-2">
          ...
          <FilterDropdown
            ...
            options={[
              { v: 'all', label: 'All' },
              ...Object.entries(TrailerTypeConfig).map(([v, c]) => ({ v, label: c.label }))
            ]}
          />
        </div>
      </div>
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-body-sm font-medium text-red-700">Error loading trailers</p>
      ...
      {!error && trailers.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-200/80 shadow-[0_1px_2px_rgba(16,24,40,0.04)] p-10 text-center">
          ...
          <p className="text-body-sm text-text-secondary mb-4">
            {allTrailers.length === 0 ? 'Add your first trailer to get started' : 'Try adjusting your search or filters'}
          </p>
          {allTrailers.length === 0 && (
        ...
        <div className="bg-white rounded-2xl border border-gray-200/80 shadow-[0_1px_2px_rgba(16,24,40,0.04)] overflow-hidden">
          <div className="divide-y divide-gray-100">
            {trailers.map((trailer) => {
              ...
              const typeConfig = trailer.typeConfig || TrailerTypeConfig[trailer.type];
              const sv = statusConfig.variant;
              const isReefer = ['reefer', 'reefer_multi_temp', 'refrigerated'].includes(trailer.type) || !!trailer.reefer_make;
              ...
              return (
                ...
                >
                  <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-accent flex items-center justify-center shrink-0">
                    <Container className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {typeConfig?.label && (
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-accent/10 text-accent border border-accent/20">
                          {typeConfig.label}
                        </span>
                      )}
                      {isReefer && (
                    ...
                    <div className="flex items-center gap-x-3 gap-y-0.5 mt-0.5 flex-wrap text-small text-text-secondary">
                      ...
                      {trailer.length_ft && <span className="text-text-tertiary">· {trailer.length_ft} ft</span>}
                      {trailer.currentTruck && (
                        <span className="flex items-center gap-1">
                          <Truck className="w-3 h-3" />
                          #{trailer.currentTruck.unit_number}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <span className={cn(
                    ...
                    {trailer.status === 'maintenance' && <Wrench className="w-3 h-3" />}
